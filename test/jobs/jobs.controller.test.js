...
describe('jobs.controller', () => {
    ...
    describe('getJobs', () => {
        it('should return a resolved promise when persistence succees', () => {
            jest.spyOn(persistence, 'getJobs').mockImplementation(() => Promise.resolve());

            return controller.getJobs();
        });

        it('should return a rejected promise when persistence fails', done => {
            jest.spyOn(persistence, 'getJobs').mockImplementation(() => Promise.reject());

            return controller.getJobs().catch(done);
        });
    });
